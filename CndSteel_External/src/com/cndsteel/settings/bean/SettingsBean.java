package com.cndsteel.settings.bean;
...
import com.cndsteel.framework.utils.ObjectCacheUtil;
...
public class SettingsBean extends BaseBean {
	...
	public synchronized static SettingsBean getInstance(){
		if(null == mSettingsBean){
			mSettingsBean = (SettingsBean)ObjectCacheUtil.readObject(Constants.CACHE_DIR + SettingsBean.class.getSimpleName().hashCode());
			if(null == mSettingsBean){
	...
	public static void save(){
		ObjectCacheUtil.cacheObject(Constants.CACHE_DIR + SettingsBean.class.getName().hashCode(), mSettingsBean);
	}
}
